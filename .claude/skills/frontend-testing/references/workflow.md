...
This guide addresses **multi-file workflow** (how to process multiple test files). For coverage requirements within a single test file, see `SKILL.md` § Coverage Goals.
...
2. Run `bun analyze <path>` (if available) or review manually
3. Check complexity score and features detected
4. Write the test file
5. Run test: `bun test <file>.spec.tsx`
6. Fix any failures
...
bun test src/path/to/directory/

# Check coverage
bun test --coverage src/path/to/directory/
```
...
Run bun test  ← Multiple failures, hard to debug
```
...
Write component-a.spec.tsx
Run bun test component-a.spec.tsx ✅
Write component-b.spec.tsx
Run bun test component-b.spec.tsx ✅
...continue...
```
...
```
Test all components in `src/path/to/directory/`.
