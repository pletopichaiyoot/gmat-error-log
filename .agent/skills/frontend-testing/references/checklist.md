...
Use this checklist when generating or reviewing tests for Bloom Mindful Companion frontend components.

## Pre-Generation

- [ ] Read the component source code completely
- [ ] Identify component type (component, hook, utility, page)
- [ ] Run `bun analyze <path>` if available (or review manually)
...
- [ ] Assess complexity manually or via script
...
- [ ] **DO NOT mock base components** (`Button`, `Input`, `Card`, etc.)
...
- [ ] Router mocks match actual `react-router-dom` API
...
- [ ] Run `bun test path/to/file.spec.tsx` - **MUST PASS before next file**
...
- [ ] Run full directory test: `bun test path/to/directory/`
- [ ] Check coverage report: `bun test --coverage`
...
# Run with coverage
bun test --coverage path/to/file.spec.tsx

# Watch mode
bun test --watch path/to/file.spec.tsx

# Update snapshots (use sparingly)
bun test -u path/to/file.spec.tsx
```
