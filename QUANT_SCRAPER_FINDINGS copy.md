...
## Will This Affect Other Question Banks?

The cumulative-categories issue is tied to **session size**, not just the bank. The pattern:

| Session structure | Categories page | Example |
|---|---|---|
| Full quiz (15–25 questions answered in one go) | Session-specific ✅ | Verbal Review |
| Small practice sets (1–5 questions per session) | Cumulative pool ⚠️ | Quant Review |

**Quick diagnostic:** After scraping any new bank, compare `total_q_categories` vs `total_q_api` for a session. If `cat >> api`, the categories page is showing cumulative data — skip the difficulty breakdown columns. If they match (±1), it's session-specific and safe to use.

The scraper handles both cases gracefully:
- `total_q_api` is always used for `# Questions` in the Practice Log
- Part 2 (review pages) is session-specific regardless of bank
- Only the difficulty breakdown (M/N columns) is unreliable when `cat >> api`
